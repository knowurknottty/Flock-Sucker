...
Java_com_arm_aichat_internal_InferenceEngineImpl_configureNative(
        JNIEnv *, jobject, jint context_size, jint batch_size, jint ubatch_size, jint decode_threads, jint batch_threads) {
    if (context_size < 256 || batch_size < 1 || ubatch_size < 1 || decode_threads < 1 || batch_threads < 1) {
        return 1;
    }
    g_context_size = context_size;
    g_batch_size = batch_size;
    g_ubatch_size = std::min(ubatch_size, batch_size);
    g_threads = decode_threads;
    g_threads_batch = batch_threads;
    LOGi("Configured context=%d batch=%d ubatch=%d threads=%d batch_threads=%d",
         g_context_size, g_batch_size, g_ubatch_size, g_threads, g_threads_batch);
    return 0;
}
...
static llama_context *init_context(llama_model *model, const int requested_ctx = -1) {
    if (!model) {
        LOGe("%s: model cannot be null", __func__);
        return nullptr;
    }

    const int n_ctx = requested_ctx > 0 ? requested_ctx : g_context_size;
    LOGi("%s: context=%d decode_threads=%d batch_threads=%d", __func__, n_ctx, g_threads, g_threads_batch);

    // Context parameters setup
    llama_context_params ctx_params = llama_context_default_params();
    const int trained_context_size = llama_model_n_ctx_train(model);
    if (n_ctx > trained_context_size) {
        LOGw("%s: Requested context %d exceeds trained context %d", __func__, n_ctx, trained_context_size);
    }
    ctx_params.n_ctx = n_ctx;
    ctx_params.n_batch = g_batch_size;
    ctx_params.n_ubatch = g_ubatch_size;
    ctx_params.n_threads = g_threads;
    ctx_params.n_threads_batch = g_threads_batch;
