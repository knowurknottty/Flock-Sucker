...
import com.inversionlabs.flocksucker.service.RogueWifiMonitor
...
fun MainViewModel.getFilteredCellularAnomalies(): List<CellularMonitor.CellularAnomaly> {
    ...
    val fpCellularDetections = state.detections.filter { detection ->
        detection.protocol == com.inversionlabs.flocksucker.data.model.DetectionProtocol.CELLULAR &&
 ...
 */
fun MainViewModel.getFilteredGnssAnomalies(): List<com.inversionlabs.flocksucker.monitoring.GnssSatelliteMonitor.GnssAnomaly> {
    val state = _uiState.value
    if (!state.hideFalsePositives) {
    ...
    val fpGnssDetections = state.detections.filter { detection ->
        detection.protocol == com.inversionlabs.flocksucker.data.model.DetectionProtocol.GNSS &&
        (detection.fpScore ?: 0f) >= state.fpFilterThreshold
    }
 ...
 */
fun MainViewModel.getFilteredSatelliteAnomalies(): List<com.inversionlabs.flocksucker.monitoring.SatelliteMonitor.SatelliteAnomaly> {
    val state = _uiState.value
    if (!state.hideFalsePositives) {
    ...
    val fpSatelliteDetections = state.detections.filter { detection ->
        detection.protocol == com.inversionlabs.flocksucker.data.model.DetectionProtocol.SATELLITE &&
        (detection.fpScore ?: 0f) >= state.fpFilterThreshold
    }
...
fun MainViewModel.getFilteredUltrasonicAnomalies(): List<UltrasonicDetector.UltrasonicAnomaly> {
    ...
    val fpUltrasonicDetections = state.detections.filter { detection ->
        detection.protocol == com.inversionlabs.flocksucker.data.model.DetectionProtocol.AUDIO &&
        (detection.fpScore ?: 0f) >= state.fpFilterThreshold
    }
...
fun MainViewModel.getFilteredRogueWifiAnomalies(): List<RogueWifiMonitor.WifiAnomaly> {
    ...
    val fpWifiDetections = state.detections.filter { detection ->
        detection.protocol == com.inversionlabs.flocksucker.data.model.DetectionProtocol.WIFI &&
        (detection.fpScore ?: 0f) >= state.fpFilterThreshold
    }
...
fun MainViewModel.getFilteredRfAnomaliesWithFp(): List<RfSignalAnalyzer.RfAnomaly> {
    ...
    val fpRfDetections = state.detections.filter { detection ->
        detection.protocol == com.inversionlabs.flocksucker.data.model.DetectionProtocol.RF &&
        (detection.fpScore ?: 0f) >= state.fpFilterThreshold
    }
