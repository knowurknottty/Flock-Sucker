...
object LlmOutputParser {
    ...
    private fun estimateConfidence(response: String, detection: Detection): Float {
        ...
        val technicalTerms = when (detection.protocol) {
            com.inversionlabs.flocksucker.data.model.DetectionProtocol.CELLULAR -> listOf("imsi", "encryption", "2g", "tower", "handoff")
            com.inversionlabs.flocksucker.data.model.DetectionProtocol.GNSS -> listOf("satellite", "spoofing", "jamming", "c/n0", "constellation")
            com.inversionlabs.flocksucker.data.model.DetectionProtocol.WIFI -> listOf("ssid", "mac", "beacon", "probe", "deauth")
            com.inversionlabs.flocksucker.data.model.DetectionProtocol.AUDIO -> listOf("ultrasonic", "frequency", "beacon", "modulation")
            else -> emptyList()
        }
